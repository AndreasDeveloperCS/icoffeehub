  ...
  if (!user) {
    return (
      <div className="container-page flex flex-col items-center gap-4 py-24 text-center">
        <span className="section-eyebrow">{t('aiAssistant.eyebrow')}</span>
        <h1 className="font-heading text-3xl font-bold text-espresso-800">{t('aiAssistant.signInTitle')}</h1>
        <p className="max-w-md text-sm text-espresso-500">{t('aiAssistant.signInBody')}</p>
  ...
  return (
    <div className="container-page py-10">
      <div className="mx-auto max-w-2xl text-center">
        <span className="section-eyebrow">{t('aiAssistant.eyebrow')}</span>
        <h1 className="mt-1.5 font-heading text-3xl font-bold text-espresso-800">{t('aiAssistant.title')}</h1>
        <p className="mt-2 text-sm text-espresso-500">{t('aiAssistant.disclaimer')}</p>
      </div>
      ...
      {tab === 'chat' ? (
        ...
        <>
          <form onSubmit={onSubmit} className="card mx-auto mt-8 max-w-2xl space-y-6 p-6 sm:p-8">
            <div>
              <label className="label">{t('aiAssistant.preferredRoasts')}</label>
            ...
            <div>
              <label className="label">{t('aiAssistant.flavorsYouLove')}</label>
              <div className="flex flex-wrap gap-2">
                {FLAVOR_NOTES.map((f) => (
            ...
            <div>
              <label className="label">{t('aiAssistant.flavorsToAvoid')}</label>
              <div className="flex flex-wrap gap-2">
                {FLAVOR_NOTES.map((f) => (
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              <SliderField label={t('aiAssistant.acidity')} value={acidity} onChange={setAcidity} lowLabel={t('aiAssistant.smooth')} highLabel={t('aiAssistant.bright')} />
              <SliderField label={t('aiAssistant.body')} value={body} onChange={setBody} lowLabel={t('aiAssistant.light')} highLabel={t('aiAssistant.full')} />
            </div>

            <button type="submit" disabled={submitting} className="btn-primary w-full text-base">
              {submitting ? t('aiAssistant.matching') : t('aiAssistant.getRecommendations')}
            </button>
          </form>

          {recommendations && (
            <div className="mt-14">
              <h2 className="text-center font-heading text-2xl font-bold text-espresso-800">{t('aiAssistant.recommendedForYou')}</h2>
              {recommendations.length === 0 ? (
                <p className="mt-4 text-center text-sm text-espresso-500">{t('aiAssistant.noMatches')}</p>
              ) : (
